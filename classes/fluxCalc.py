...
# ## (2ND) LOOP THROUGH FLUX SURFACES TO SHIFT DATA AND SPLINE FIT ## PLOTTING EACH FLUX SURFACE AT EACH PHI ANGLE 
# # Set fluxes outside of LCFS to be equal to the LCFS flux, and set range of data to be between 0 and the LCFS flux ## OUTPUT
'''
...
class FluxCalculator:
    ...
    def process_all_phi_angles(self):
        for phi_index, PHI_GEN_DEG in enumerate(self.phi_gens):
            flux_surfaces = self.load_poincare_data(PHI_GEN_DEG)
            # INITIALIZE PLOTTING
            if self.plot_all: fig, ax_rect, ax_hist, ax_polar = self.init_plotting()
            ...
            if phi_index == 0:
                ...
                self.valid_surfs = np.zeros([self.nsurfaces, len(self.phi_gens)], dtype=bool) # keep track of valid surfaces
                self.valid_surfs[self.lcfs_index:][:] = True  # set all surfaces after LCFS to valid

            ## FIND THE MAGNETIC AXIS FROM SMALLEST FLUX SURFACE (ASSUMED LAST FLUX SURFACE IN SET)
            mag_axis = self.find_axis(*flux_surfaces[-1], self.b_hidra)
            mag_axis_rev = np.copy(mag_axis)
            mag_axis_rev[0] += np.pi

            ## LOOP THROUGH FLUX SURFACES TO FIND SUBSETS (ISLAND) AND THE SET OF SMALLEST ISLANDS
            first_loop_output = self.first_surface_loop(flux_surfaces, mag_axis, self.b_hidra, self.lcfs_index, self.nsurfaces, self.max_subsets)
            ...
            for surf_index in range(self.lcfs_index, self.nsurfaces):
                ...
                output = self.subset_looper(subsetData, subsetCenters, surf_index,
                                        smallest_island_index, N_subsets, wrap_flag,
                                        self.b_hidra, mag_axis_rev, self.ntheta, PHI_GEN_DEG,
                                        self.integrate_epsabs, self.integrate_epsrel, self.smooth_fctr, self.simIO)
                points_tr_geoAxis, spline_tr_magAxis, subCenters_geo, subset_flux_list, self.valid_surfs[surf_index] = output

                ## APPENDING LISTS AND RESHAPING ARRAYS
                total_npts = num_subsets[surf_index] * self.ntheta
                ...
                if surf_index == self.lcfs_index:
                    ...
                    if not self.big_mesh:
                        self.lcfs_flat_point[phi_index][:NOW_NPTS] = LCFSdata_list
                lcfs_radii = axisShift(*LCFSdata_list.T, *mag_axis_rev)[1]

                if num_subsets[surf_index] == self.max_subsets: #if num_subsets[surf_index] > 1:
                    plot_centers_geo = self.centers_array[surf_index][phi_index].T
                else:
                    plot_centers_geo = self.centers_array[surf_index][phi_index][0].T

                # Filter out wild fits
                delta_plot_rs = np.max(lcfs_radii) - np.max(plot_radii)
                if delta_plot_rs > 0.: # Add points to output arrays
                    if self.big_mesh: self.flat_point_meshes[surf_index][phi_index][:NOW_NPTS] = plot_tr_points
                    if self.plot_all:
            ...
            for flux_index, flux in enumerate(Fluxes, start=self.lcfs_index):
                self.tot_flux_array[flux_index][phi_index] = np.sum(flux, axis=-1)

            if self.plot_all: self.finalize_plotting(fig, ax_rect, ax_hist, ax_polar, PHI_GEN_DEG, num_subsets, self.max_subsets, self.simIO)
        ...
        return self.tot_flux_array, self.valid_surfs, smallest_island_index, self.lcfs_flat_point , self.flat_point_meshes


    def finalize_plotting(self, fig, axRect, axHist, axPolar, PHI_GEN_DEG, num_subsets, MAX_SUBSETS, simIO):
        ...
        simIO.saveFig(self.anlys_subdir +'/Splines_{:03d}deg.png'.format(int(PHI_GEN_DEG)), dpi=300)
        plt.close()
    
    def find_axis(self, theta_vals: np.ndarray, r_vals: np.ndarray, field: Mesh, alt=False) -> np.ndarray:
        """
        Computes the geometric center (axis) of a set of points in (r, theta) coordinates.

        The functsimIOn filters NaNs from the input arrays, converts the remaining(r, theta)
        ...
        Args:
            ...
            field (Mesh): Mesh object with attribute R0, used for coordinate transformatsimIOns.

        Returns:
            axis_thetar (np.ndarray): Array containing the axis positsimIOn in (theta, r) coordinates.
        """
        ...
        # Vectorized coordinate transformatsimIOn instead of loop
    ...
    def find_subsets(self,max_subsets, theta_r_pts, mag_axis, field, BINS=120):
        ...
        for i, contiguous_set in enumerate(contiguous_sets):
            ...
            if num_sets == max_subsets:
                found_centers[i][:] = self.find_axis(thisSet_tr.T[0], thisSet_tr.T[1], field, alt=True)
    ...
    def first_surface_loop(self,flux_surfaces, mag_axis, field, start_index, end_index, max_subsets):
        """
        ...
        for surf_index in range(start_index, end_index):
            ...
            output_histogram_method = self.find_subsets(max_subsets, pts_tr_magAxis, mag_axis, field, BINS=self.hist_bins)
    ...
    def init_plotting(self):
        """
        Initialize the plotting for the flux calculatsimIOn.
        """
        width_in = 15 / 2.54 # 15 cm in inches
        height_in = width_in * (9/16)  # Maintain 16:9 aspect ratsimIO
    ...
    def shift_the_subcenters(self, surf_index, smallest_island_index, subsetCenters, num_subsets, wrap_flag):
    ...
    def subset_looper(self, subsetData, subsetCenters, surf_index,
                  smallest_island_index, num_subsets, wrap_flag,
                  field, mag_axis_rev, NTHETA, PHI_GEN_DEG,
                  INTEGRATE_EPSABS, INTEGRATE_EPSREL, SMOOTH_FCTR, simsimIO
                  ):
        """
        ...
        Args:
            ...
            NTHETA (int): Number of theta points for evaluatsimIOn.
            PHI_GEN_DEG (float): Phi angle in degrees.
            INTEGRATE_EPSABS (float): Absolute error tolerance for integratsimIOn.
            INTEGRATE_EPSREL (float): Relative error tolerance for integratsimIOn.
            SMOOTH_FCTR (float): Smoothing factor for spline fitting.
            simsimIO: SimulatsimIOn I/O object for logging and saving data.
        ...
        Returns:
            ...
            valid_surface (bool): Flag indicating if the surface is valid for interpolatsimIOn.
        """
        ...
        if num_subsets > 1: 
            subCenters_Shift, shiftint = self.shift_the_subcenters(surf_index, smallest_island_index, subsetCenters, num_subsets, wrap_flag[surf_index] )
        else: shiftint = 0
        ...
        for subset_idx in range(num_subsets):
            ...
            ## SPLINE FIT
            fSurface_splineParms, res, fail, msg = self.spline_data(*current_data.T, smoothing=SMOOTH_FCTR)
            if fail:
                self.valid_surface = False # not a valid surface for interpolatsimIOn
                simsimIO.log.info('\t(B)Surface #{} NOT A VALID SURFACE!!! Residual: {}'.format(surf_index, res) )
                simsimIO.log.info('\tSurface #{}, fail: {}\tmsg: {}'.format(surf_index, bool(fail), msg) )
                continue
            else:
                simsimIO.log.info( '\t(B)Surface #{} valid. Residual: {:.3e}'.format(surf_index, res) )
                self.valid_surface = True # valid surface for interpolatsimIOn

            ## INTEGRATE AMOUNT OF TOROIDAL FIELD BOUNDED BY FLUX SURFACE [Tesla*m^2]
            this_flux = self.integrate_flux(fSurface_splineParms, np.array(current_center, copy=True),
        ...
        return splined_tr_GeoAxis, splined_tr_MagAxis, subCenters_geo, subset_flux_list, self.valid_surface
    ...
    def flux_normalization(self):
        ...
        total_flux_norm = 1 - self.tot_flux_array / self.tot_flux_array[self.lcfs_index]
        total_flux_norm = np.clip(total_flux_norm, 0.0, 1.0)
        total_flux_norm[:self.lcfs_index][:] = 0.0

        return total_flux_norm
    
    def perform_output(self, smallest_island_index):
        ## OUTPUT FLUXES
        ...
        for i in range(len(self.phi_gens)):
            axPSI.plot(self.tot_flux_array[:,i]*10_000, label='{:d}'.format(int(self.phi_gens[i])), linewidth=1)
            axNORM.plot(self.total_flux_norm.T[i], label='{:d}'.format(int(self.phi_gens[i])), linewidth=1)
        axPSI.set_title('Toroidal Angles $\phi[\degree]$', loc='right', fontsize=8)
        axPSI.set_ylim(0, 1.1*10_000*np.max(self.tot_flux_array)) 
        ...
        self.simIO.log.info('Finished, LCFS=#{}, ISLAND AXIS=#{}'.format(self.lcfs_index, smallest_island_index))

        # SAVE THE NUMPY ARRAYS TO INDIVIDUAL FILES USING SIMsimIO METHOD
        for surf_index in range(self.lcfs_index, self.nsurfaces):
    ...
    def run(self):
        self.tot_flux_array, self.valid_surfs, smallest_island_index, self.lcfs_flat_point, self.flat_point_meshes = self.process_all_phi_angles()

        self.total_flux_norm = self.flux_normalization()

        self.perform_output(smallest_island_index)

        return self.tot_flux_array, self.total_flux_norm, self.valid_surfs
