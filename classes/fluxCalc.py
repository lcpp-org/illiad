...
class FluxCalculator:
    ...
    def process_surfaces_for_phi(self, phi_index, phi_deg, flux_surfaces, mag_axis, 
        ...
        For each surface from the LCFS inward, this method fits spline to the surface/subsets, 
        integrates the toroidal flux enclosed by each subset, and stores the resulting flux values. 
        It also updates center locations, surface-validity flags, plots, and regularly-spaced 
        point meshes used for later output.
        ...
        for surf_index in range(self.lcfs_index, self.nsurfaces):
            ...
            output = self.fit_and_integrate_surface_subsets(subset_data, subset_centers, surf_index, smallest_island_index, 
                                        n_subsets, wrap_flag, mag_axis_rev, phi_deg)
            points_tr_geoAxis, spline_tr_magAxis, subCenters_geo, subset_flux_list, valid_surface = output
            # storing flux and center data
            ...
            if surf_index == self.lcfs_index:
                ...
                self.surface_validation(surf_index, phi_index,
                                        ...
                                        n_subsets,
                                        spline_lcfs_magaxis)
            # plot each flux surface at each phi angle
            if self.plot_all:
                self.plot_flux_surface_for_phi(surf_index, phi_index, flux_surfaces, 
                                               mag_axis, spline_tr_magAxis, num_subsets, 
                                               ax_rect, ax_hist, bin_edges, hist)
            # flatten surface points into one theta-r list
            ...
            # store lcfs data and shift its central axis to magnetic axis
            if surf_index == self.lcfs_index:
                LCFS_data_list = surface_tr_points
                if not self.big_mesh:
                    self.lcfs_flat_point[phi_index][:total_npts] = LCFS_data_list
            lcfs_radii = axisShift(*LCFS_data_list.T, *mag_axis_rev)[1]
            ...
            delta_surface_radius = np.max(lcfs_radii) - np.max(surface_radii)
            if delta_surface_radius > 0.0: # Add points to output arrays
                if self.big_mesh: 
    ...
    def plot_flux_surface_for_phi(self, surf_index, phi_index, flux_surfaces, 
                                  mag_axis, spline_tr_magAxis, num_subsets, 
                                  ax_rect, ax_hist, bin_edges, hist):
        """Plot one flux surface and its spline fit for a single toroidal angle.
        
        Notes:
            The raw Poincare points are shifted to the magnetic axis frame and plotted
            in the rectangular theta r view."""
        th_in, r_in = flux_surfaces[surf_index]
        r_in = r_in[~np.isnan(r_in)]
        th_in = th_in[~np.isnan(th_in)]
        ...
        if self.valid_surfs[surf_index][phi_index]:
            ...
            ax_hist.bar(bin_edges[surf_index][:-1]*180.0/np.pi, hist[surf_index],
                width=np.diff(bin_edges[surf_index])*180.0/np.pi, align='edge', edgecolor='k', linewidth=0.1)
