...
expand_dr   = [0.020, 0.030, 0.040]                                     # define number of 'shells' (delta-r) to generate
ntheta      = 60                                          # number of equally-spaced theta points for each shell
...
for phi_gen_deg in phiGen_list:
    ...
    seed_subset = generateSeedShells(expand_dr, ntheta, r_in, th_in, phi_gen, 
        b_hidra, simOut, 'SeedPts_{}mm'.format(dr_String))
    seed_list.extend(seed_subset)

spins = 400
length = (2*np.pi * b_hidra.R0) * spins
seed_array = [fieldLine(seed_pt, length) for seed_pt in seed_list]
...
subName = 'SeedPts_{}mm'.format(dr_String)
...
simOut.saveFig('Wallpoints_SeedPts_{}mm'.format(dr_String))
