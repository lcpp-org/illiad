...
ntheta      = 2                                          # number of equally-spaced theta points for each shell
...
spins = 4
#max_life = (2*np.pi * b_hidra.R0) * spins
max_life = 1. # seconds
Li_mass = 3. #amu

seed_array = [Ion(seed_pt, Li_mass, +1, max_life) for seed_pt in seed_list]
init_v_XYZ = np.array([0., 0., 1000.])
[ion.initialize_velocity(init_v_XYZ) for ion in seed_array]
...
tMax2, Poincare_output2, wallPt_output2 = Gen_Poincare(b_hidra, seed_array, simOut, subName, 'RK45', 1e-5, 1e-32, saveData=False)
