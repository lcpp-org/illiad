...
def main():
    ...
    plot_name = "bfield_fitting_040126"
    ...
    B_rtp_onlyScale = np.zeros((len(PHI_PLOT), 3))
    B_rtp_LF = np.zeros((len(PHI_PLOT), 3))
    B_rtp_LF_ideal = np.zeros((len(PHI_PLOT), 3))
    B_rtp_LF_onlyScale = np.zeros((len(PHI_PLOT), 3))
    ...
    fig = plt.figure()
    axSolo = fig.add_subplot(111)
    lf_ideal, = axSolo.plot(PHI_PLOT, np.sqrt( B_rtp_LF_ideal[:,0]**2 + B_rtp_LF_ideal[:,1]**2 + B_rtp_LF_ideal[:,2]**2),
                             '--k', label='Ideal Field', zorder=5)
    lf_scale, = axSolo.plot(PHI_PLOT, np.sqrt( B_rtp_LF_onlyScale[:,0]**2 + B_rtp_LF_onlyScale[:,1]**2 + B_rtp_LF_onlyScale[:,2]**2),
                             ':g', label='w/ scalars', zorder=5)
    ...
    axSolo.set_ylabel('$\| \mathbf{B} \| \:[G]$')
    axSolo.grid(which='both')
    axSolo.legend(handles=[lf_ideal, lf_scale, lf_fit, lf_data], loc='upper right')

    axSolo.set_xlim(0, 360)
    axSolo.set_xticks(np.arange(0, 361, 90))
    plt.tight_layout()
    simIO.saveFig('solo_' + plot_name + '_lf.png', dpi=400)
