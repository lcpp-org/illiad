...
def fluxCalculator():
    ...
    for phi_index, PHI_GEN_DEG in enumerate(PHI_GENs):
        ...
        if PLOT_ALL: fig, ax1, ax2, ax4 = init_plotting()
        ...
        for surf_index in range(LCFS_INDEX, NSURFACE):
            ...
            if np.any(spline_tr_magAxis[:,:,1] >= 0.13) or np.any(spline_tr_magAxis[:,:,1] < 0.0):
            ...
            if PLOT_ALL:
                ...
                ax1.scatter(points_tr_magAxis.T[0]*180./np.pi, points_tr_magAxis.T[1], color='k', s=0.25, linewidths=0.0) # mag-axis point
                # plot the spline fit and histogram
                if valid_surfs[surf_index]:
                    for i in range(0, num_subsets[surf_index]):
                        ax1.scatter(spline_tr_magAxis[i].T[0]*180./np.pi, spline_tr_magAxis[i].T[1], s=0.5, linewidths=0.05)    
            ...
            if delta_plot_rs > 0.: # Add points to output arrays
                ...
                if PLOT_ALL:
                    ...
                    if plot_centers_geo.shape[-1] > 1:
                        ax4.scatter(plot_centers_geo[0], plot_centers_geo[1], s=15, color='k', marker='x',  linewidths=0.5)
        ...
        if PLOT_ALL: finalize_plotting(fig, ax1, ax2, ax4,
                                        PHI_GEN_DEG, num_subsets, MAX_SUBSETS, simIO)
    ...
    axUP.set_ylabel('Toroidal Flux $\psi_{\phi}$\n$[g*m^2]$', fontsize=8)
...
def find_Axis(theta_vals: np.ndarray, r_vals: np.ndarray, field: Mesh, alt=False) -> np.ndarray:
    ...
    coords_3d = np.array([RTP_to_XYZ(np.array([r, theta, 0.]), field.R0) 
...
def find_subsets(max_subsets, theta_r_pts, mag_axis, field, BINS=120):
    ...
    for i, contiguous_set in enumerate(contiguous_sets):
        ...
        # only split if the number of sets is equal to the maximum number of subsets: 
...
def subset_looper(subsetData, subsetCenters, surf_index,
    ...
    for subset_idx in range(num_subsets):
        ...
        # # TEST PLOT
        # test_fig = plt.figure('polar')
        # ax = test_fig.add_subplot(111, polar=True)
        # ax.scatter(current_data.T[0], current_data.T[1], s=5)
        # plt.show()
        # nan_mask = np.isnan(current_data.T[1])
        # print('# of NaNs={}'.format(np.sum(nan_mask)))
        ...
        if fail:
            ...
            simIO.log.info( '\t(B)Surface #{} NOT A VALID SURFACE!!!'.format(surf_index) )
        ...
        # # TEST PLOT 2
        # test_fig2 = plt.figure('polar2')
        # ax2 = test_fig2.add_subplot(111, polar=True)
        # ax2.plot(current_theta_gens, radius_evals_locAxis[subset_idx], c='orange')
        # plt.show()
...
def finalize_plotting(fig, ax1, ax2, ax4, PHI_GEN_DEG, num_subsets, MAX_SUBSETS, simIO):
    ...
    print('Plotting Flux Surfaces @ phi={}'.format(PHI_GEN_DEG))
    ax4.set_rlim(0, 0.19)
...
if __name__ == '__main__':
    ...
    # ANLYS_DIR = "ChangeToIota4_1500spins_ohhhhhhhhhhhhhh"
    # ANLYS_SUBDIR = 'LCFS29_3x360x360mesh_CORRECTCURR_lotol'
