...
def integrate_flux(spline_parms, spline_axis, phi, field, err_abs=1e-5, err_rel=1e-3):
    ...
    def flux_integrand(r, theta, phi, field, axis):
        ...
        # The integrand is -r*B_toroidal = -r*( Bx*sin(phi) - By*cos(phi) )
        #return -r*(bxyz[0]* np.sin(phi) - bxyz[1]*np.cos(phi))
        return r*(bxyz[1]*np.cos(phi) - bxyz[0]* np.sin(phi))
...
def subset_looper(subsetData, subsetCenters, surf_index,
    ...
    if num_subsets > 1: 
        subCenters_Shift, shiftint = shift_the_subcenters(surf_index, smallest_island_index,
                                                           subsetCenters, num_subsets, wrap_flag[surf_index] )
    else: shiftint = 0
    ...
    for subset_idx in range(num_subsets):
        ...
        for theta_idx, theta in enumerate(current_theta_gens):
            if num_subsets > 1:
                ...
                splined_tr_MagAxis[subset_idx][theta_idx] = axisShift(theta, 
                                                                      radius_evals_locAxis[subset_idx][theta_idx], 
                                                                      shift_theta, shift_r)
            else:
...
if __name__ == '__main__':
    ...
    ANLYS_SUBDIR = "LCFS39_4x36x360mesh_OLDclstr"
