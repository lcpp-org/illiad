"""Command-line adapter for SOL electrostatic-potential construction."""

import argparse

from illiad.io import IOHandler
from illiad.sol import SOLPotential
...
DEFAULT_INPUTS = {
    ...
    "ANLYS_SUBDIR": (
        "ConLenVolume_REDO_250spins_rk1mm_RegularGrid_Stitched_v2-3"
    ),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Construct an ILLIAD piecewise core/SOL electrostatic-potential "
            "field."
        )
    )
    parser.add_argument(
...
def main(input_overrides=_CLI_INPUTS):
    if input_overrides is _CLI_INPUTS:
        args = parse_args()
        input_overrides = (
            load_inputs_json(args.inputs, "SOL potential inputs")
    ...
    sim_io.startLog(
        ...
        logger_name="SOLPotential",
    )
    analysis = SOLPotential(sim_io, params)
    analysis.run()

    print(f"Saved potential field: {analysis.output_path}")
    print(f"Saved potential metadata: {analysis.metadata_path}")
    if params["GENERATE_PLOTS"]:
        print(f"Saved potential plots: {sim_io.plot_dir}/{params['ANLYS_SUBDIR']}")
