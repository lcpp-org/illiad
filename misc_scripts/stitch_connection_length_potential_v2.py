    ...
    if requested_value is not None:
        ...
        if not isinstance(spins, int) or spins <= 0:
            raise ValueError(
                ...
                "SPINS value in the Poincare log; provide --l-parallel-0-m."
            )
...
def validate_settings(delta_phi_0w, delta_phi_sol, alpha, sol_beta):
    ...
    if (
        LAMBDA_PHI_MIN_M is not None
        and LAMBDA_PHI_MAX_M is not None
        and LAMBDA_PHI_MIN_M > LAMBDA_PHI_MAX_M
    ):
        raise ValueError(
            "LAMBDA_PHI_MIN_M cannot exceed LAMBDA_PHI_MAX_M."
        )
    if (
        PLOT_VMIN is not None
        and PLOT_VMAX is not None
        and PLOT_VMIN >= PLOT_VMAX
    ):
...
def load_inputs(analysis_dir, sol_subdir, nfield_subdir, nfield_filename):
    base_data_dir = PROJECT_ROOT / "output" / analysis_dir / "data"
    sol_data_dir = base_data_dir / sol_subdir
    nfield_path = base_data_dir / nfield_subdir / nfield_filename

    sol_path = require_file(
        sol_data_dir / SOL_FIELD_FILENAME,
        "regular connection-length field",
    )
    ...
    require_file(nfield_path, "interior nField")

    sol = np.load(sol_path, mmap_mode="r")
    ...
    if sol.shape != expected_shape:
        raise ValueError(
            f"SOL field shape {sol.shape} does not match coordinates "
            f"{expected_shape}."
        )
    if nfield.shape != expected_shape:
        raise ValueError(
            f"nField shape {nfield.shape} does not match SOL field shape "
            f"{expected_shape}. Regenerate both fields on the same mesh."
        )
    if sol.ndim != 3:
    ...
    if not np.all(np.isfinite(nfield)):
        raise ValueError("nField contains non-finite values.")

    finite_sol = np.isfinite(sol) & (sol > 0.0)
    if not np.any(finite_sol):
        raise ValueError(
            "Connection-length field has no positive finite samples."
        )
    return sol, nfield, rho, theta, phi_deg, sol_path, nfield_path
...
def points_inside_boundary(boundary, points, shape):
    closed = np.vstack((boundary, boundary[0]))
    return MplPath(closed).contains_points(points).reshape(shape)
...
def resample_closed_curve(vertices, point_count):
    ...
    return np.column_stack(
        (
            np.interp(target, cumulative, closed[:, 0]),
            np.interp(target, cumulative, closed[:, 1]),
        )
    )


def outward_normals(boundary):
    tangent = np.roll(boundary, -1, axis=0) - np.roll(
        boundary, 1, axis=0
    )
...
def periodic_polar_interpolator(theta, rho, values):
    theta_extended = np.concatenate(
        ([theta[-1] - 2.0 * np.pi], theta, [theta[0] + 2.0 * np.pi])
    )
...
def wall_distance(origins, directions, vessel_radius):
    ...
    discriminant = (
        origin_dot_direction**2
        + vessel_radius**2
        - np.sum(origins**2, axis=1)
    )
...
def apply_profile_alpha(linear_nfield, alpha):
    """Transform q=1-Psi_tor into 1-Psi_tor**alpha."""
    linear_profile = np.clip(
        np.asarray(linear_nfield, dtype=np.float64),
        0.0,
        1.0,
    )
    return 1.0 - (1.0 - linear_profile) ** alpha


def surface_profile_slope(
    nfield_interpolator,
    boundary,
    normals,
    ...
    if not np.all(np.isfinite(profile_one)) or not np.all(
        np.isfinite(profile_two)
    ):
        raise ValueError(
            "Could not evaluate the nField on both inward derivative shells."
        )
    ...
    if not np.all(np.isfinite(slope)) or np.any(slope <= 0.0):
        ...
        raise ValueError(
            f"Derived nonpositive LCFS nField slope at {bad_count} surface "
            "points. Verify that the selected LCFS matches the nField."
        )
...
def bridge_connection_length_paths(raw_paths, path_distance, l_parallel_0):
    ...
    if np.any(~np.any(valid, axis=1)):
        ...
        raise ValueError(
            f"No positive exterior connection length was found on {count} "
            "LCFS-to-wall paths."
        )
    ...
    fraction = np.divide(
        path_distance,
        first_distance[:, None],
        out=np.ones_like(path_distance),
        where=first_distance[:, None] > 0.0,
    )
    bridged = l_parallel_0 + (
        first_value[:, None] - l_parallel_0
    ) * smoothstep(fraction)
    ...
    for row in np.flatnonzero(np.any(invalid_after, axis=1)):
        ...
        bridged[row, ~good] = np.interp(
            path_distance[row, ~good],
            path_distance[row, good],
            bridged[row, good],
        )
...
def construct_path_attenuation(
    sol_plane,
    profile_plane,
    theta,
    rho,
    ...
    nfield_interpolator = periodic_polar_interpolator(
        theta,
        rho,
        profile_plane,
    )
    slope, profile_one, profile_two = surface_profile_slope(
        nfield_interpolator,
        boundary,
        normals,
        derivative_step,
    ...
    path_points = boundary[:, None, :] + (
        path_distance[:, :, None] * normals[:, None, :]
    )

    sol_interpolator = periodic_polar_interpolator(
        theta,
        rho,
        np.asarray(sol_plane, dtype=np.float64),
    )
    raw_connection = sol_interpolator(
        xz_to_theta_rho(path_points.reshape(-1, 2))
    ).reshape(path_distance.shape)
    connection, bridge_width = bridge_connection_length_paths(
        raw_connection,
        path_distance,
        l_parallel_0,
    )

    lambda_phi = lambda_phi_0[:, None] * (
        connection / l_parallel_0
    ) ** sol_beta
    ...
    increments = 0.5 * (
        inverse_lambda[:, 1:] + inverse_lambda[:, :-1]
    ) * np.diff(path_distance, axis=1)
...
def evaluate_exterior_profile(
    ...
    origins = boundary[surface_index]
    displacement = exterior_points - origins
    distance = np.linalg.norm(displacement, axis=1)
    if np.any(distance == 0.0):
        directions = normals[surface_index].copy()
        directions[distance > 0.0] = (
            displacement[distance > 0.0] / distance[distance > 0.0, None]
        )
    else:
    ...
    chi_target = (
        (1.0 - weight) * chi[surface_index, lower]
        + weight * chi[surface_index, upper]
    )
    ...
    profile = outer_value + profile_difference * (
        exp_target - exp_wall
    ) / denominator
    return profile
...
def construct_plane(
    ...
    boundary = resample_closed_curve(boundary, BOUNDARY_RESAMPLE_POINTS)
    normals = outward_normals(boundary)
    inside = points_inside_boundary(
        boundary,
        grid_points,
        (theta.size, rho.size),
    )
    ...
    interior_profile = apply_profile_alpha(nfield_plane, alpha)
    output[inside] = (
        PHI_WALL
        + delta_phi_sol
        + delta_phi_core * interior_profile[inside]
    )

    chi, diagnostics = construct_path_attenuation(
        sol_plane,
        interior_profile,
        theta,
        rho,
    ...
    output[~inside] = evaluate_exterior_profile(
...
def build_piecewise_field(
    ...
    progress = tqdm(
        range(phi_deg.size),
        desc="Constructing piecewise potential",
        unit="plane",
        dynamic_ncols=True,
    ...
    try:
        with log_context:
            for plane_index in progress:
                ...
                plane, diagnostics = construct_plane(
                    sol[plane_index],
                    nfield[plane_index],
                    theta,
                    rho,
                ...
                boundary_all[plane_index] = diagnostics["boundary"]
                normal_all[plane_index] = diagnostics["normal"]
                lambda_phi_0_all[plane_index] = diagnostics["lambda_phi_0"]
                lambda_phi_min_all[plane_index] = diagnostics["lambda_phi_min"]
                lambda_phi_max_all[plane_index] = diagnostics["lambda_phi_max"]
                slope_all[plane_index] = diagnostics["slope"]
                wall_distance_all[plane_index] = diagnostics[
                    "path_wall_distance"
                ]
                bridge_width_all[plane_index] = diagnostics["bridge_width"]
                chi_wall_all[plane_index] = diagnostics["chi_wall"]
                sim_io.log.info(
                    "Constructed phi=%03.0f: %d interior/%d exterior cells, "
                    "lambda_phi_0 %.6g/%.6g/%.6g m, bridge "
                    "%.6g/%.6g/%.6g m, chi_w min %.6g, potential "
                    "%.6g to %.6g.",
                    phi_deg[plane_index],
                    ...
                    diagnostics["potential_min"],
                    diagnostics["potential_max"],
                )
                if plane_index % 10 == 0:
                    output.flush()
                    gc.collect()
    ...
    sim_io.log.info(
        "Constructed %d planes in %.3f s (%.3f s/plane).",
        phi_deg.size,
        elapsed,
...
def plot_plane(
    ...
    ax.set_title(
        "Piecewise electrostatic potential\n"
        f"$\\phi_{{comp}}={phi_deg:.0f}^\\circ$, "
...
def generate_plots(
    ...
    progress = tqdm(
        range(phi_deg.size),
        desc="Plotting piecewise potential",
        unit="plane",
        dynamic_ncols=True,
        disable=not show_progress,
    )
...
def nearest_coordinate_index(coordinates, target, name):
    ...
    if abs(coordinates[index] - target) > tolerance:
        raise ValueError(
            f"Requested {name}={target:g} is not represented by the grid."
        )
    return index


def generate_midplane_trace_plot(
    field,
    rho,
    theta,
    phi_deg,
    ...
    theta_lfs_index = nearest_coordinate_index(
        theta,
        2.0 * np.pi,
        "theta_LFS",
    )
    ...
    for requested_phi in MIDPLANE_TRACE_PHI_DEG:
        phi_index = nearest_coordinate_index(
            phi_deg,
            requested_phi,
            "phi_comp",
        )
    ...
    ax.axhline(
        ...
        linestyle="--",
        linewidth=1.2,
        label=rf"$\Phi_{{SOL}}={phi_sol_normalized:g}$",
    )
...
def main():
    ...
    output_subdir = (
        ...
        else f"{args.sol_subdir}_Stitched_v2"
    )

    poincare_settings = load_poincare_settings(args.analysis_dir)
    lcfs_index, lcfs_index_source = resolve_lcfs_index(
        args.lcfs_index,
        args.nfield_file,
        poincare_settings,
    )
    ...
    if PLOT_VMIN is None:
        plot_vmin = (
            LOG_PLOT_VMIN if args.color_scale == "log" else PHI_WALL
        )
    else:
        plot_vmin = float(PLOT_VMIN)
    plot_vmax = (
        PHI_WALL + args.delta_phi_0w
        if PLOT_VMAX is None
        else float(PLOT_VMAX)
    )
    ...
    run_settings = {
        ...
        "NFIELD_DATA_MIN": float(np.min(nfield)),
        "NFIELD_DATA_MAX": float(np.max(nfield)),
    ...
    sim_io.inputsBoilerplate(
        "PIECEWISE NFIELD / CONNECTION-LENGTH POTENTIAL INPUTS",
        run_settings,
        list(run_settings),
    )
    ...
    print(f"Reading interior nField: {nfield_path}")
    print(
        f"Using LCFS surface {lcfs_index} ({lcfs_index_source}) and "
        f"L_parallel,0={l_parallel_0:.6g} m"
    )
    print(f"Output field shape (phi, theta, rho): {sol.shape}")
    field, metadata = build_piecewise_field(
        args.analysis_dir,
        sol,
        nfield,
        rho,
    ...
    sim_io.log.info(
        "Finite-wall LCFS slope factor 1/(1-exp(-chi_w)): %.6g to %.6g.",
        np.min(1.0 / (-np.expm1(-metadata["chi_wall"]))),
        np.max(1.0 / (-np.expm1(-metadata["chi_wall"]))),
    )

    if args.plots:
        generate_midplane_trace_plot(
            field,
            rho,
            theta,
            phi_deg,
        ...
        sim_io.log.info(
            "Saved horizontal-midplane potential trace: %s/%s.",
            output_subdir,
            MIDPLANE_TRACE_FILENAME,
        )
        generate_plots(
            field,
            rho,
        ...
        sim_io.log.info(
            "Saved %d piecewise-potential plots under %s.",
            phi_deg.size,
            Path(sim_io.plot_dir) / output_subdir,
        )

    sim_io.log.info(
        "## PIECEWISE NFIELD / CONNECTION-LENGTH POTENTIAL FINISHED ##"
    )
    print(f"Saved piecewise potential: {output_path}")
