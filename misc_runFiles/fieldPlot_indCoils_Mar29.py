...
def main():
    ...
    b_TCoils.loadCartesianField('input_files/It486_Ih000_Iv000_1p000_1p000.npy', errField=True)
    b_TCoils.err_mag = 0.0
    b_TCoils.err_dir = 0.0
    b_TCoils.att_mult = 1.0
    # Load Helical Field Coils
    b_HCoils = Mesh(R0=0.72, a=0.19)
    b_HCoils.loadCartesianField('input_files/It000_Ih900_Iv000_1p000_1p000.npy', errField=True)
    b_HCoils.err_mag = 0.0
    b_HCoils.err_dir = 0.0
    ...
    def compBfield(phi_deg, errMag=0.0, errDir=0.0, torMult=1.0, helMult=1.0):
        ...
        b_TCoils.att_mult = torMult

        b_HCoils.err_mag = errMag
        b_HCoils.err_dir = errDir
        b_HCoils.att_mult = -helMult
    ...
    def return_Bnorm(independents, errMag=0.0, errDir=0.0, torMult=1.0): #, helMult=1.0):
        ...
        b_HCoils.err_mag = errMag
        b_HCoils.err_dir = errDir
        b_HCoils.att_mult = -torMult*0.955 #helMult
    ...
    plot_name = "Mar29Fit_r0_dBdr-1mm_noHEL.png"
    #plot_title = "Fitting only r=0 points:"
    plot_title = "Fitting r=0 Measurement points:"
    ...
    calc_torMult = popt[2]
    calc_helMult = 0.955 * popt[2] # popt[3]
    ...
    plt.rcParams.update({'font.size': 4, 'axes.labelsize': 4, 'legend.fontsize': 4, 'axes.titlesize': 5, 'lines.linewidth': 0.6})
    fig = plt.figure()
    fig.suptitle(plot_title+'\n'+resultString, fontsize=6)

    ax1 = fig.add_subplot(221)
    ax1.plot(PHI_PLOT, np.sqrt( B_rtp_ideal[:,0]**2 + B_rtp_ideal[:,1]**2 + B_rtp_ideal[:,2]**2), '--k', label='Ideal (no error field)', zorder=4)
    ax1.plot(PHI_PLOT, np.sqrt( B_rtp[:,0]**2 + B_rtp[:,1]**2 + B_rtp[:,2]**2), 'b', label='Fitted Error Field', zorder=5)
    ax1.plot(PHI_PLOT, np.sqrt( B_rtp_plus[:,0]**2 + B_rtp_plus[:,1]**2 + B_rtp_plus[:,2]**2), 'g', label='plus 10cm', zorder=5)
    ax1.plot(PHI_PLOT, np.sqrt( B_rtp_HF[:,0]**2 + B_rtp_HF[:,1]**2 + B_rtp_HF[:,2]**2), 'r', label='plus 10cm HF', zorder=5)
    ax1.errorbar( np.degrees(ind_measured[:,1]), dep_measured, yerr=sigma_fitPts,
                  fmt='s', markersize=1.0, color='k', capsize=2, ecolor='k', zorder=5)
    ax1.set_ylabel('Scalar Field Strength [Gauss]')
    ax1.grid(which='both')

    ax2 = fig.add_subplot(222)
    ax2.plot(PHI_PLOT, B_rtp_ideal[:, 2], '--k', label='Ideal (no error field)', zorder=4)    
    ax2.plot(PHI_PLOT, B_rtp[:, 2], 'b', label='Fitted Error Field', zorder=5)
    ax2.plot(PHI_PLOT, B_rtp_plus[:, 2], 'g', label='Fitted 10cm LF', zorder=5)
    ax2.plot(PHI_PLOT, B_rtp_HF[:, 2], 'r', label='Fitted 10cm HF', zorder=5)
    ax2.errorbar(np.degrees(measurements[:, 1]), measurements[:, 2], yerr=np.sqrt(sigma_measured**2 + dBtor_dArm**2 + dBtor_dChip**2 + dBtor_dMount**2),
                  fmt='s', markersize=1.0, color='k', capsize=2, ecolor='k', zorder=5)
    ax2.set_ylabel('Toroidal (Z) Field Strength [Gauss]')
    ...
    ax3.errorbar(np.degrees(measurements[:, 1]), measurements[:, 3], yerr=np.sqrt(sigma_measured**2 + dBrad_dArm**2 + dBrad_dChip**2 +  dBrad_dMount**2),
                  fmt='s', markersize=1.0, color='k', capsize=2, ecolor='k', zorder=5)
    ax3.set_xlabel('Toroidal Angle [deg], +CW from North Split')
    ax3.set_ylabel('Radial (X) Field Strength [Gauss]')
    ax3.grid(which='both')
    ...
    ax4.errorbar(np.degrees(measurements[:, 1]), measurements[:, 4], yerr=np.sqrt(sigma_measured**2 + dBpol_dArm**2 + dBpol_dChip**2 + dBpol_dMount**2),
                  fmt='s', markersize=1.0, color='k', capsize=2, ecolor='r', zorder=5)   
