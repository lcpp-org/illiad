...
simOut = out.IOHandler("1q3-ERR_Poincare_PHI324_LSODA_1e-8")
simOut.startLog()
...
#b_hidra.loadCartesianField_fromFile('Bxyz_iota-1q3_MAXPOWER_hires_95p5pct.npy', 0, 1, 5)
...
fl_R0     = np.array(np.linspace( 0.140, 0.000, Nlines))
...
ntheta = 180 # number of equally-spaced theta points for each shell
...
for phi_gen_deg in phiGen_list:
    ...
    seed_subset = generateSeedShells(expand_dr, ntheta, r_in, th_in, phi_gen, b_hidra, simOut, 'SeedPts_2cm')##########################
...
tMax2, Poincare_output2, wallPt_output2 = Gen_Poincare(b_hidra, seed_array, length, simOut, False, 'SeedPts_2cm')##############################
...
simOut.saveNumpyData(wallPtArray, 'Wallpoints_2cm')######################################################################
...
simOut.saveFig('Wallpoints_SeedPts_2cm') ######################################################################
